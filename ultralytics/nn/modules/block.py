...
class SelFusion(nn.Module):
    """Implementation for LiteUAV-Det."""
    def __init__(self, c, div=4):
        """
        Args:
            c (int): Number of input channels.
            div (int): The division count for PConv. FasterNet tests 2, 4 and 8.
        """
        super().__init__()
        self.cbl1 = Conv(c, c, k=3, s=1, p=None, act=nn.ReLU())
        self.pconv1 = PConv(c, div, k=3, s=1, p=1)
        self.cbl2 = Conv(c, c, k=3, s=1, p=None, act=nn.ReLU())
        self.pconv2 = PConv(c, div, k=3, s=1, p=1)
        self.cbl3 = Conv(c, c, k=3, s=1, p=None, act=nn.ReLU())
        self.pointwise = Conv(c, c, k=1, s=1, p=0)

    def forward(self, x):
        """
        Figure 5 in https://doi.org/10.1016/j.neucom.2026.133782
        """
        skip = x
        x = self.pconv1(self.cbl1(x))
        x = self.pconv2(self.cbl2(x))
        x = self.pointwise(self.cbl3(x))
        return skip + x


class DSFF(nn.Module):
    """Implementation for LiteUAV-Det."""
    def __init__(self, c1, c2, n, div=4):
        """
        Args:
            c1 (int): Number of input channels.
            c2 (int): Number of output channels.
            n (int): Number of SelFusion blocks
            div (int): The division count for PConv. FasterNet tests 2, 4 and 8.
        """
        super().__init__()
        c_ = c2 // 2
        self.conv1 = Conv(c1, c_, 1, 1, None)
        self.bn1 = nn.BatchNorm2d(c_)
        self.conv2 = Conv(c_, c_, 3, 1, None)
        self.selfusion_stack = nn.ModuleList([SelFusion(c_//2, div) for _ in range(n)])
        self.conv3 = Conv(int(0.5*(n+2)*c_), c_, 3, 1, None)
        self.bn2 = nn.BatchNorm2d(c_)
        self.conv4 = Conv(c_, c2, 1, 1, None)

    def forward(self, x):
        x = self.conv2(self.bn1(self.conv1(x)))
        split1, split2 = torch.split(x, x.shape[1] // 2, dim=1)
        x = split1
        skip1, skip2 = split1, split2
        skip_from_selfusion = [None for _ in range(len(self.selfusion_stack))]
        for i, selfusion in enumerate(self.selfusion_stack):
            x = selfusion(x)
            skip_from_selfusion[i] = x
        x = torch.cat([skip1, skip2, *skip_from_selfusion], 1)  # (H,W,0.5*(n+2)*c2)
        x = self.conv4(self.bn2(self.conv3(x)))
        return x
    

class EPPF(nn.Module):
    """Implementation for LiteUAV-Det."""

    def __init__(self, c1, c2):
        """Initialize the EPPF layer with given input/output channels and kernel size.
        ...
        super().__init__()
        c_ = c1 // 2  # hidden channels
        self.conv1 = Conv(c1, c_, 1, 1, 0)
        self.conv2 = Conv(c_, c_, 3, 1, None)
        self.m = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
        self.conv3 = Conv(c_*4, c_, 3, 1, None)
        self.conv4 = Conv(c_, c2, 1, 1, 0)

    def forward(self, x):
        y = [self.conv2(self.conv1(x))]
        y.extend(self.m(y[-1]) for _ in range(3))
        y = self.conv4(self.conv3(torch.cat(y, 1)))
        return y
