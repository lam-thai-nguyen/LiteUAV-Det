...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
        elif m is PConv or m is SelFusion:
            """Implementation for LiteUAV-Det."""
            c = ch[f]
            args = [c, *args]
        elif m is DSFF:
            """Implementation for LiteUAV-Det."""
            c1, c2, num_selfusion = ch[f], args[0], args[1]
            c2 = make_divisible(min(c2, max_channels) * width, 8)
            num_selfusion = max(round(num_selfusion * depth), 1) if num_selfusion > 1 else num_selfusion
            args = [c1, c2, num_selfusion, *args[2:]]
        elif m is EPPF:
            """Implementation for LiteUAV-Det."""
            c1, c2 = ch[f], args[0]
            c2 = make_divisible(min(c2, max_channels) * width, 8)
